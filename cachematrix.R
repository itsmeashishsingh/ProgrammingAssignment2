...
## I have written two methods to cache inverse of a matrix
...
## setinverse function ( look at the call, its calling the list elements by name and element happens to
## be a function ). setinverse stores output in inv and returns the value to console.
...
## cached, so we need to call makeCacheMatrix which sets inv to NULL so now when you call cacheSolve 
